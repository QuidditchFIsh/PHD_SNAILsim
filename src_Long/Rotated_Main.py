...
def H1_Coeff_On(t,*args):
	...
	return cos((-cons.omega1) * t + (PI*0.5))
print("============================================")
...
#R = 1/sqrt(2) * (sigmaz() + sigmax())
R = 1
a = R * destroy(2) * R

#Define the operators 
q1 = tensor(a,qeye(N),qeye(N))
q2 = tensor(qeye(N),a,qeye(N))
q3 = tensor(qeye(N),qeye(N),a)
...
sz1 = tensor(R * sigmaz()* R,qeye(N),qeye(N))
sz2 = tensor(qeye(N),R * sigmaz() * R,qeye(N))
sz3 = tensor(qeye(N),qeye(N),R * sigmaz() * R)
