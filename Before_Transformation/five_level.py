...
H0 = omega1 * q1.dag()*q1 + omega2 * q2.dag()*q2 + omega3 * q3.dag()*q3 + omegaM * sM.dag() * sM + U1 * q1.dag() * q1.dag() * q1 * q1 + U2 * q2.dag() * q2.dag() * q2 * q2 + U3 * q3.dag() * q3.dag() * q3 * q3
'''
mult  = 0.01
mult2 = 0.01
...
'''
#X = Identity + (0.825* (sM + sM.dag())) - (0.125 * (sM + sM.dag())**2)
...
'''
coeff1_12_coeff 	= 3  		* E_J * phiAC * X
coeff1_3_coeff  	=-6 		* E_J * phiAC * X
	
coeff2_1_coeff  	= 1 		* E_J * phiAC * X
coeff2_23_coeff		=-2 		* E_J * phiAC * X

coeff3_coeff    	= 0.666667  * E_J * phiAC * X
'''
...
sx3 = tensor(i,i, R * sigmax() * R,i)

sy1 = tensor(R 		* sigmay() * R,i,i,i)
sy2 = tensor(i, R 	* sigmay() * R,i,i)
sy3 = tensor(i,i, R * sigmay() * R,i)

sz1 = tensor(R 		* sigmaz() * R,i,i,i)
sz2 = tensor(i, R 	* sigmaz() * R,i,i)
sz3 = tensor(i,i, R * sigmaz() * R,i)
sz4 = tensor(i,i,i,sigmaz())
#sz5 = tensor(i,i,i,i,sigmaz())

c_ops = [0.001*q1,0.001*q2,0.001*q3,0.002*sz1,0.002*sz2,0.002*sz3,0.002*sz4,0.001*sM]
...
result = mesolve(H,psi0,tlist,c_ops,[sx1,sy1,sz1,sx2,sy2,sz2,sx3,sy3,sz3,sz4],options = Options(nsteps = 2**14,store_states = True,store_final_state = True))
